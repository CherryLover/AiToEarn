...
interface NotifyTarget {
   ...
   *
   * - `user`：用户在网页上填的，内网一律拒
   * - `env`：服务器 `.env` 渲染进来的，运维自己配的，**放行内网地址但不放行跨主机的跳转**
   *   （线上就有把 Bark 装在同机、写成 `http://127.0.0.1:.../` 的用法，一刀切拦掉会把既有部署打死；
   *   但跟着对端给的 302 跑到别的主机，运维从来没同意过——见 `notify.http.ts` 的说明）
   */
  source: 'user' | 'env'
}
...
export class NotifyService {
   ...
   *
   * 唯一的区别是 `allowPrivateAddress`：`.env` 那条允许地址本身落在内网
   * （运维可能把 Bark 装在同机），但**跟着 302 换到别的主机就重新按严格规则判**。
