...
| `CONFIG_OVERRIDE_ERROR_CODE`      | `object` | 覆盖层错误码（20800 段，逐个对齐）。            |
| `SYSTEM_READINESS_ERROR_CODE`     | `object` | 就绪检查错误码（20850 段，逐个对齐）。          |
...
- 单项探测失败回在 `items[].status`，不走错误码；20850 段说的是检查本身出错。
