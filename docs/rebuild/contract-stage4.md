...
1. **读草稿文件，把内容快照下来**（走 safe-fs）。骨架第四节写明：**必须用快照，不能只存路径**——草稿在排队期间可能被改，发出去的应该是点「准备发布」那一刻的版本

   **草稿有两种形态，都必须支持**：
   - **目录版** `drafts/<slug>/content.md` + `meta.json` —— AI 生成的走这种，血缘在 meta.json 里
   - **单文件版** `drafts/<名字>.md` —— 人手工放进来的走这种，血缘（如果有）写在 frontmatter 里

   > 这条是踩过坑才加的：第一版只认目录版，结果线上用户手工放的三份单文件草稿在发布页里一份都用不了——网页把它们列进了下拉框，一点就报错。**判断方式看磁盘上是文件还是目录，不要靠路径有没有 `.md` 后缀去猜。**
